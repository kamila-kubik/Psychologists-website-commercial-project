...
(function() {
  ...
  Comp.fn = Library.prototype = {
      ...
      while (len--) {
        ...
        this[len].setAttribute(
          ...
          this[len].getBoundingClientRect().top +
            (window.pageYOffset || document.documentElement.scrollTop) -
            (document.documentElement.clientTop || 0) +
            parseInt(getComputedStyle(this[len]).marginTop) +
            parseInt(getComputedStyle(this[len]).marginBottom) +
            this[len].offsetHeight
        );
      }
    ...
    changeClass: function(firstClassName, secondClassName) {
      //card
      var len = this.length;
      ...
      while (len--) {
        for (var i = 0; i < classArrayLength; i++) {
          if (this[len].classList) {
            ...
            this[len].className = this[len].className.replace(
              new RegExp(
                "(^|\\b)" + className[i].split(" ").join("|") + "(\\b|$)",
                "gi"
              ),
              " "
            );
